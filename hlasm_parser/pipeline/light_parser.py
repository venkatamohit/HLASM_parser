...
class LightParser:
    ...
    def _split_statement(line: str) -> tuple[str, str, str]:
        ...
        if len(parts) == 1:
            tok = parts[0]
            if tok.upper() in _COL1_OPCODE_HINTS:
                return "", tok, ""
            return tok, "", ""
        if parts[0].upper() in _COL1_OPCODE_HINTS:
            opcode = parts[0]
            operands = body.split(opcode, 1)[1].strip() if len(parts) > 1 else ""
            return "", opcode, operands
        label = parts[0]
